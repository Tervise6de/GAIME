...
function generatedLayout(rng) {
  ...
  // DIFFICULTY NORMALIZATION: the guard (and thus the rich pile, the bulk of
  // the quota) must stay within projectable range of the nest. A guard in the
  // far corner cannot be cleared before the colony bleeds out — a competence
  // sweep showed those maps are the unwinnable tail. Keep the objective
  // reachable; regenerate otherwise. (Reference: the handcrafted seed-7 guard
  // sits ~460px from the nest; the cap allows well beyond that.)
  if (Math.hypot(guard.hx - nest.x, guard.hy - nest.y) > 1010) return null;
  let mid = null;
  for (let tries = 0; tries < 80; tries++) {
