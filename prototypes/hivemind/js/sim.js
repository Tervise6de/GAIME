...
const FREE_SPAWNS = 1400;    // colony's initial reserves
const SPAWN_COST = 0.25;     // food per replacement ant afterwards
...
export function step(s, dt) {
  ...
  const target = Math.min(MAX_ANTS, 900 + Math.floor(s.time * 28)); // ramp in
  ...
  for (let i = 0; i < s.count; i++) {
    ...
    // promotion / demotion to soldier
    if (state !== ST.SOLDIER && warHere > 0.25) s.astate[i] = ST.SOLDIER;
    else if (state === ST.SOLDIER && warHere < 0.05) s.astate[i] = ST.FORAGE;
    ...
    if (s.astate[i] === ST.RETURN) {
      ...
      if (dx * dx + dy * dy < nest.r * nest.r) {
        if (s.acarry[i]) { s.foodBanked++; s.foodStock++; s.acarry[i] = 0; }
      ...
      for (const p of piles) {
        ...
        if (dx * dx + dy * dy < rr * rr) {
          p.amount--; s.acarry[i] = 1; s.astate[i] = ST.RETURN;
